...
const folderPath = 'assets/distinct'
const baseImageUrl = 'https://1cms-img.imgix.net'
const errorLogPath = 'batch_errors2.txt'
const downloadLogPath = 'download_log.txt'
...
function cleanFileName(fileName) {
  ...
  const ext = path.extname(cleaned) || '.jpg' // Default to .jpg if no extension
              ...
              const validImages = results
                .map((item, index) => {
                  ...
                  if (uniqueImages.has(item.image)) {
                    logToFile(
                      `Batch ${i}, row ${index}: Duplicate image: ${item.image}`,
                      errorLogPath
                    )
                    return null // Skip duplicates (shouldn't happen with DISTINCT)
                  } else {
